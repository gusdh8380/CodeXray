...
The system SHALL reuse the existing inventory walk/ignore rules and language classification. 그래프 노드는 `inventory` capability가 정의한 1차 매핑(Python, JavaScript, TypeScript, Java, C#) 중 1차 변경 대상 3개 언어(Python, JavaScript, TypeScript) 파일에 한정한다.
...
#### Scenario: 비대상 언어 파일
- **WHEN** 입력 디렉토리에 Java(`.java`) 또는 C#(`.cs`) 파일이 존재할 때 `codexray graph`를 실행하면
